...
def get_resource_template_and_static_paths():
    ...
    directories named templates/static inside the one-file extraction root. In the
    source tree those assets remain under app/templates and app/static.
    """
    ...
    if not os.getenv('VERCEL'):
        with app.app_context():
            ...
            try:
                ...
                # Keep legacy sale tables compatible with the newer document-aware workflow.
                if 'document_type' not in sale_columns:
                    if dialect == 'sqlite':
                        db.session.execute(text("ALTER TABLE sale ADD COLUMN document_type VARCHAR(20) NOT NULL DEFAULT 'receipt'"))
                    elif dialect == 'postgresql':
                        db.session.execute(text("ALTER TABLE sale ADD COLUMN IF NOT EXISTS document_type VARCHAR(20) NOT NULL DEFAULT 'receipt'"))

                if 'discount' not in sale_columns:
                    if dialect == 'sqlite':
                        db.session.execute(text("ALTER TABLE sale ADD COLUMN discount FLOAT NOT NULL DEFAULT 0.00"))
                    elif dialect == 'postgresql':
                        db.session.execute(text("ALTER TABLE sale ADD COLUMN IF NOT EXISTS discount FLOAT NOT NULL DEFAULT 0.00"))

                if 'deposit' not in sale_columns:
                    if dialect == 'sqlite':
                        db.session.execute(text("ALTER TABLE sale ADD COLUMN deposit FLOAT NOT NULL DEFAULT 0.00"))
                    elif dialect == 'postgresql':
